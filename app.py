...
if df_full.empty:
    ...
    with c_bar2:
        ...
        st.download_button(label="📥 Bajar Excel", data=excel_data, file_name="Reporte_Financiero.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    with g_col2:
        ...
        if not df_view.empty:
            ...
            if st.button("🖨️ Crear PDF"):
                ...
                pdf_bytes = crear_recibo_pdf(fila['FECHA'].strftime('%Y-%m-%d'), fila['BANCO'], abs(fila['IMPORTE']), fila['DESCRIPCION'])
